...
총 3차에 걸쳐 개선하며 품질을 측정했습니다.

**평가 조건 변화**

| 차수 | 변경 내용 |
|---|---|
| 1차 (기본) | 기본 프롬프트, 벡터 검색만 사용 |
| 2차 (프롬프트 강화) | if-else 구조 + `[문서무관]` 태그 도입 |
| 3차 (Hybrid Search) | BM25 + 벡터 RRF 결합 검색 적용 |

**2차 프롬프트 강화 내용**

1차에서는 단순히 "문서를 참고해서 답하세요"만 지시했으나, 문서에 없는 질문에서 "답변할 수 없습니다"만 반환하는 문제가 발생했습니다. 이를 해결하기 위해 명확한 if-else 구조로 재작성했습니다.
...
| Context Precision | 0.96 | 0.97 | 0.55 | 일반 HR 질문 추가로 평균 하락 (아래 참고) |
| Answer Relevancy | 측정 불가 | 측정 불가 | 측정 불가 | 아래 참고 |

> **핵심**: Faithfulness 0.24 → 0.65로 170% 향상. 프롬프트 강화 + Hybrid Search(BM25+벡터 RRF)의 복합 효과.

**Context Precision 하락 원인**

Context Precision은 "검색된 청크가 질문과 얼마나 관련 있는가"를 측정합니다. 3차에서 일반 HR 질문(예: "퇴직금은 얼마예요?")을 데이터셋에 추가했는데, 이 질문들은 문서에 없는 내용이라 검색 시 관련 없는 청크가 반환됩니다. 이로 인해 전체 평균이 낮아졌으며, 더 현실적인 사용 환경을 반영한 결과로 판단했습니다.
